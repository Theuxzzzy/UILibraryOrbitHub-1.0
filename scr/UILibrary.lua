...
function OrbitHub:Notify(opts)
    ...
    OrbitHub._notifCount = OrbitHub._notifCount + 1
    ...
    Tween(card, { BackgroundTransparency = 0.15 }, 0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
    Tween(progressBar, { Size = UDim2.new(0, 0, 0, 2) }, dur, Enum.EasingStyle.Linear)
    task.delay(dur, function()
        Tween(card, { BackgroundTransparency = 1, Position = UDim2.new(0, 300, 0, 0) }, 0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.In)
        task.wait(0.35)
        card:Destroy()
    end)
end
function OrbitHub:CreateWindow(opts)
    ...
    local tSelectItem = New("Frame", {
        Name = "TemplateDeSelecionar",
        BackgroundColor3 = Color3.fromRGB(13, 13, 13),
        BackgroundTransparency = 0.4,
        BorderSizePixel = 0,
        Size = UDim2.new(0, 108, 0, 25),
        Visible = false,
        Parent = templates,
    })
    New("UICorner", { Parent = tSelectItem })
    New("TextLabel", {
        Name = "Title",
        BackgroundTransparency = 1,
        BorderSizePixel = 0,
    ...
    function Window:AddTab(name, icon)
        ...
        function Tab:AddButton(opts)
            ...
            c.SubTitle.Text = opts.SubTitle or ""
        ...
        function Tab:AddSlider(opts)
            ...
            function S:Set(v)
                ...
                Tween(ball, { Position = UDim2.new(pct, -6, -1.2, 0) }, 0.15)
        ...
        function Tab:AddDropdown(opts)
            ...
            local selected = opts.Default or (items[1] or "Nenhum")
            local open = false
            ...
            New("UIListLayout", { SortOrder = Enum.SortOrder.LayoutOrder, Padding = UDim.new(0, 2), Parent = dropFrame })
            local function buildItems()
                for _, ch in ipairs(dropFrame:GetChildren()) do
                    if ch:IsA("Frame") then ch:Destroy() end
                end
                for i, item in ipairs(items) do
                    local row = Clone(tSelectItem, dropFrame)
                    row.Title.Text = item
            ...
            c.Button.MouseButton1Click:Connect(function()
                ...
                if open then
                    ...
                    c.Info.Text = selected .. " ▴"
                    Tween(dropFrame, { Size = UDim2.new(1, 0, 0, math.min(totalH, 120)) }, 0.2)
                else
                    c.Info.Text = selected .. " ▾"
                    Tween(dropFrame, { Size = UDim2.new(1, 0, 0, 0) }, 0.15)
                    task.delay(0.16, function() dropFrame.Visible = false end)
            ...
            local D = {}
            function D:Set(v) selected = v; c.Info.Text = v .. " ▾"; end
            function D:Get() return selected end
            function D:SetItems(list) items = list; totalH = #list * 27; buildItems() end
            return D
        end
